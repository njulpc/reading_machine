...
def load_real_qwen():
    ...
    try:
        ...
        print("[GPTQ-2D] Loading real Qwen3-0.6B from Hugging Face...")
        ...
        print("[GPTQ-2D] Real model loaded successfully.")
        return model, tokenizer
    except Exception as e:
        print(f"[GPTQ-2D] Could not load real model: {e}")
        return None, None


def apply_gptq2d_to_model(model, bits=4, max_layers=3, max_dim=1024):
    """Apply GPTQ-2D quantization to a subset of small nn.Linear layers for demo speed."""
    # Skip large layers (embed_tokens, lm_head) to avoid excessive CPU time.
    # GPTQ-2D on a 151936x576 matrix would take hours in pure Python.
    quantizer = GPTQ2D(bits=bits)
    count = 0
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) and count < max_layers:
            out_f, in_f = module.weight.shape
            # Skip overly large layers for CPU demo
            if out_f > max_dim or in_f > max_dim:
                print(f"  Skipping {name}: [{out_f}, {in_f}] (too large for CPU demo)")
                continue
            # Use simplified identity Hessian for demo
            H_A = torch.eye(out_f, device=module.weight.device)
            H_B = torch.eye(in_f, device=module.weight.device)
            A = torch.linalg.cholesky(H_A)
            B = torch.linalg.cholesky(H_B)
            
            w_q = quantizer.quantize(module.weight.data, A, B)
            module.weight.data.copy_(w_q)
            count += 1
            print(f"  Quantized {name}: [{out_f}, {in_f}]")
    
    print(f"[GPTQ-2D] Quantized {count} layers.")
    return model
...
def demo():
    ...
    # --------------------------------------------------------------------------
    # Part 4: Real Qwen3-0.6B (optional, requires transformers + HF access)
    # --------------------------------------------------------------------------
    print("\n" + "-" * 70)
    print(" [PART 4] Real Qwen3-0.6B (optional — requires transformers + network)")
    print("-" * 70)
    ...
    if real_model is not None:
        ...
        with torch.no_grad():
            fp_out = real_model(input_ids).logits
        print(f"  FP16 forward pass OK, logits shape: {fp_out.shape}")
        
        # GPTQ-2D on real model weights:
        # Full GPTQ-2D quantization of a real Qwen3-0.6B (1024-dim layers) on CPU
        # takes tens of minutes per layer in pure Python. For demo purposes we
        # verify the algorithm on a small extracted slice instead, and note that
        # production deployment requires custom GPU kernels.
        print("\n[GPTQ-2D] Verifying algorithm on a small weight slice from real model...")
        # Extract a small 32x32 slice from the first quantizable layer
        for name, module in real_model.named_modules():
            if isinstance(module, nn.Linear):
                w_slice = module.weight.data[:32, :32].clone()
                A = torch.eye(32)
                B = torch.eye(32)
                quantizer = GPTQ2D(bits=4)
                w_q = quantizer.quantize(w_slice, A, B)
                print(f"  Slice quantization OK on {name}[:32,:32]")
                print(f"  Shape: {w_slice.shape}, max diff vs round: {(w_q - w_slice.round().clamp(-7,7)).abs().max().item():.4f}")
                break
        
        print("\n[GPTQ-2D] Note: Full model weight quantization on CPU is prohibitively slow")
        print("  (O(m³) per layer, ~minutes per 1024×1024 matrix in Python).")
        print("  Production use requires custom GPU kernels for anti-diagonal parallelism.")
    else:
