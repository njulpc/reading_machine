...
class HiFloat4Quantizer:
    ...
    def _round_to_fp4(self, x_norm: torch.Tensor) -> torch.Tensor:
        ...
        Input values are expected to be in the range [-6, 6] after
        hierarchical scaling (tensor/block/sub-block levels map to FP4 range).
        """
        # Values are in [-6, 6] after hierarchical scaling (FP4 dynamic range)
...
def load_real_qwen():
    ...
    try:
        ...
        print("[HiFloat4] Loading real Qwen3-0.6B from Hugging Face...")
        ...
        print("[HiFloat4] Real model loaded successfully.")
        return model, tokenizer
    except Exception as e:
        print(f"[HiFloat4] Could not load real model: {e}")
        return None, None


def apply_hif4_to_model(model, use_resq=False, max_layers=5):
    """Apply HiFloat4 quantization to all nn.Linear layers in a model."""
    quantizer = HiFloat4Quantizer()
    resq = RolloutResQ() if use_resq else None
    
    count = 0
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) and count < max_layers:
            if isinstance(module, nn.Linear):
                w_q, meta = quantizer.quantize(module.weight.data)
                if resq is not None:
                    w_q = resq.apply(w_q, module.weight.data)
                module.weight.data.copy_(w_q)
    
            count += 1
            print(f"  Quantized {name}: {tuple(module.weight.shape)}")
    
    print(f"[HiFloat4] Quantized {count} layers.")
    return model
    return model
...
def demo():
    ...
    if real_model is not None:
        ...
        # Apply HiFloat4 quantization
        print("\n[HiFloat4] Applying HiFloat4 quantization to real Qwen3-0.6B...")
        apply_hif4_to_model(real_model, use_resq=False, max_layers=3)
        
        # Forward pass with quantized weights
        with torch.no_grad():
            q_out = real_model(input_ids).logits
        
        cos = torch.nn.functional.cosine_similarity(fp_out.reshape(-1), q_out.reshape(-1), dim=0)
