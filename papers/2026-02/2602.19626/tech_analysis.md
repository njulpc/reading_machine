...
1. 提出了可命名的新方法/框架 **Nacrith**，属于量化（Quantization）、KV Cache 压缩、高效架构设计方向的新方案；
...
**摘要中出现的关键数值**（去重后）：0, 0.723, 0.918 bit, 0.9389, 06, 1, 1.024, 1.11, 1.2 GB, 100 MB, 11.74%, 135, 15%, 152, 16, 2, 2.5x, 21, 24, 29
...
4. 本文（Nacrith）表明即通过降低权重/激活的数值精度来压缩模型体积、降低显存占用并加速推理，是大模型部署的核心技术之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
