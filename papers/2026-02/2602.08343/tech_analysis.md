...
本文属于**KV Cache 压缩**方向的研究，提出了名为 **WindowedManifoldKV** 的方法，在 RULER 等基准上进行了验证。
...
本文提出 WindowedManifoldKV：We propose ManifoldKV, a training-free scorer that ranks tokens by Euclidean distance to the key centroid, capturing both angular and radial deviations.（摘要原文）
...
1. 提出了可命名的新方法/框架 **WindowedManifoldKV**，属于KV Cache 压缩方向的新方案；
...
4. 本文（WindowedManifoldKV）表明长上下文与长推理链场景下，KV Cache 的显存占用随序列长度线性增长，已成为大模型推理部署的主要瓶颈之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
