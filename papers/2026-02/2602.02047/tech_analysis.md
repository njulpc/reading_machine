...
本文属于**量化（Quantization）、硬件加速/软硬件协同**方向的研究，提出了名为 **Hot-Channel** 的方法。

> 论文摘要首句：*"Training large language models using 4-bit arithmetic enhances throughput and memory efficiency."*

### 1.2 一句话总结

本文提出 Hot-Channel：We find that, compared with Softmax Attention (SA), Linear Attention (LA) reduces per-tensor heavy tails but still exhibits persistent block-level spikes under block quantization.（摘要原文）
...
1. 提出了可命名的新方法/框架 **Hot-Channel**，属于量化（Quantization）、硬件加速/软硬件协同方向的新方案；
...
**摘要中出现的关键数值**（去重后）：0.58%, 0.94%, 1.3, 16, 4, 60
...
4. 本文（Hot-Channel）表明即通过降低权重/激活的数值精度来压缩模型体积、降低显存占用并加速推理，是大模型部署的核心技术之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
