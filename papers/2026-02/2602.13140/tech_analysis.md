...
1. 提出了可命名的新方法/框架 **FlashSchNet**，属于量化（Quantization）方向的新方案；
...
**摘要中出现的关键数值**（去重后）：1, 1000, 16, 2, 269, 3, 4, 6.5x, 6000, 64, 80%
...
4. 本文（FlashSchNet）表明即通过降低权重/激活的数值精度来压缩模型体积、降低显存占用并加速推理，是大模型部署的核心技术之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
