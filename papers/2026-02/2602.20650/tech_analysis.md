...
本文属于**量化（Quantization）**方向的研究，提出了名为 **Dataset** 的方法，在 CIFAR-10、CIFAR-100、ImageNet、ImageNet-1K 等基准上进行了验证。
...
本文提出 Dataset：To address this, we propose Dataset Color Quantization (DCQ), a unified framework that compresses visual datasets by reducing color-space redundancy while preserving information crucial for model training.（摘要原文）
...
1. 提出了可命名的新方法/框架 **Dataset**，属于量化（Quantization）方向的新方案；
...
- *"Extensive experiments across CIFAR-10, CIFAR-100, Tiny-ImageNet, and ImageNet-1K show that DCQ significantly improves training performance under aggressive compression, offering a scalable and robust solution for dataset-level storage reduction."*

**摘要中出现的关键数值**（去重后）：1, 10, 100
...
4. 本文（Dataset）表明即通过降低权重/激活的数值精度来压缩模型体积、降低显存占用并加速推理，是大模型部署的核心技术之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
