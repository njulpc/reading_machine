# 深度技术分析：SPQ: An Ensemble Technique for Large Language Model Compression

> **论文信息**
> - **arXiv ID**: 2602.18420
> - **标题**: SPQ: An Ensemble Technique for Large Language Model Compression
> - **作者**: Jiamin Yao, Eren Gultepe
> - **提交日期**: 2026-02-20
> - **分类**: cs.CL
> - **链接**: https://arxiv.org/abs/2602.18420
> - **代码**: https://github.com/JiaminYao/SPQ_LLM_Compression/
...
本文属于**量化（Quantization）、剪枝（Pruning）、低秩分解/低秩适应（Low-Rank）**方向的研究，提出了名为 **SPQ** 的方法，目标模型/架构涉及 GPTQ、LLaMA-2-7B，在 GSM8K、TruthfulQA、WikiText-2 等基准上进行了验证。

> 论文摘要首句：*"This study presents an ensemble technique, SPQ (SVD-Pruning-Quantization), for large language model (LLM) compression that combines variance-retained singular value decomposition (SVD), activation-based pruning, and post-training linear quantization."*

### 1.2 一句话总结

本文提出 SPQ：This study presents an ensemble technique, SPQ (SVD-Pruning-Quantization), for large language model (LLM) compression that combines variance-retained singular value decomposition (SVD), activation-based pruning, and post-training linear quantization.（摘要原文）
...
1. 提出了可命名的新方法/框架 **SPQ**，属于量化（Quantization）、剪枝（Pruning）、低秩分解/低秩适应（Low-Rank）方向的新方案；
...
- **涉及基准/数据集**: GSM8K、TruthfulQA、WikiText-2
