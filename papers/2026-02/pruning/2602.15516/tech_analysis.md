# 深度技术分析：Semantic-Guided 3D Gaussian Splatting for Transient Object Removal
...
本文属于**剪枝（Pruning）**方向的研究，目标模型/架构涉及 CLIP。

> 论文摘要首句：*"Transient objects in casual multi-view captures cause ghosting artifacts in 3D Gaussian Splatting (3DGS) reconstruction."*

### 1.2 一句话总结

本文围绕剪枝（Pruning）开展研究，Existing solutions relied on scene decomposition at significant memory cost or on motion-based heuristics that were vulnerable to parallax ambiguity.（摘要原文）
...
- *"CLIP similarity scores between rendered views and distractor text prompts were accumulated per-Gaussian across training iterations."*
