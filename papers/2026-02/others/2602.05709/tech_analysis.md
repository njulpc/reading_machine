# 深度技术分析：Nonlinearity as Rank: Generative Low-Rank Adapter with Radial Basis Functions

> **论文信息**
> - **arXiv ID**: 2602.05709
> - **标题**: Nonlinearity as Rank: Generative Low-Rank Adapter with Radial Basis Functions
> - **作者**: Yihao Ouyang, Shiwei Li, Haozhao Wang, Xiandi Luo, Zhuoqi Hu, Yuetong Song 等
> - **提交日期**: 5 Feb 2026 (v1), last revised 19 May 2026 (this version, v2)
> - **分类**: cs.AI
...
- *"Low-rank adaptation (LoRA) approximates the update of a pretrained weight matrix using the product of two low-rank matrices."*
- *"However, standard LoRA follows an explicit-rank paradigm, where increasing model capacity requires adding more rows or columns (ie, basis vectors) to the low-rank matrices, leading to substantial parameter growth."*
...
- *"In this paper, we find that these basis vectors exhibit significant parameter redundancy and can be compactly represented by lightweight nonlinear functions."*
- *"Therefore, we propose Generative Low-Rank Adapter (GenLoRA), which replaces explicit basis vector storage with nonlinear basis vector generation."*
- *"Specifically, GenLoRA maintains a latent vector for each low-rank matrix and employs a set of lightweight radial basis functions (RBFs) to synthesize the basis vectors."*
