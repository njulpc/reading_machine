# 深度技术分析：Enhancing Automatic Chord Recognition via Pseudo-Labeling and Knowledge Distillation

> **论文信息**
> - **arXiv ID**: 2602.19778
> - **标题**: Enhancing Automatic Chord Recognition via Pseudo-Labeling and Knowledge Distillation
> - **作者**: Nghia Phan, Rong Jin, Gang Liu, Xiao Dong
> - **提交日期**: 2026-02-23
> - **分类**: cs.IR, cs.LG, cs.MM, cs.SD
...
> 论文摘要首句：*"Automatic Chord Recognition (ACR) is constrained by the scarcity of aligned chord annotations, which are costly to acquire."*

### 1.2 一句话总结

本文In this work, we present a two-stage training pipeline that leverages pre-trained models together with unlabeled audio.（摘要原文）
...
- *"Automatic Chord Recognition (ACR) is constrained by the scarcity of aligned chord annotations, which are costly to acquire."*
- *"At the same time, open-weight pre-trained models are more accessible than their proprietary training data."*
- *"In this work, we present a two-stage training pipeline that leverages pre-trained models together with unlabeled audio."*
- *"The proposed method decouples training into two stages."*
...
- *"In this work, we present a two-stage training pipeline that leverages pre-trained models together with unlabeled audio."*
- *"In our experiments, two models (BTC, 2E1D) were used as students."*
