...
本文属于**量化（Quantization）**方向的研究，提出了名为 **UG** 的方法。
...
本文提出 UG：In this work, we present User-Group Separation (UG-Sep), an industrial large-scale framework that enables user-side computation reusable in TokenMixer-based dense interaction models for the first time.（摘要原文）
...
1. 提出了可命名的新方法/框架 **UG**，属于量化（Quantization）方向的新方案；
...
4. 本文提出的 UG 在量化（Quantization）方向提供了可直接借鉴的具体设计（见第 3 节原文引用），复现并与本文结果对比是切入该方向的低成本路径。
