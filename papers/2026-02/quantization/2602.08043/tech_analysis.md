# 深度技术分析：V-ABFT: Variance-Based Adaptive Threshold for Fault-Tolerant Matrix Multiplication in Mixed-Precision Deep Learning
...
本文属于**量化（Quantization）**方向的研究，提出了名为 **V-ABFT** 的方法，目标模型/架构涉及 GPT-2、LLaMA-7B、ViT。

> 论文摘要首句：*"Algorithm-Based Fault Tolerance (ABFT) is widely adopted to detect silent data corruptions (SDCs) in matrix multiplication, a cornerstone operation in deep learning systems."*

### 1.2 一句话总结

本文提出 V-ABFT：We present V-ABFT, a variance-based adaptive threshold algorithm that achieves tighter error bounds by directly modeling the verification difference.（摘要原文）
...
- *"Algorithm-Based Fault Tolerance (ABFT) is widely adopted to detect silent data corruptions (SDCs) in matrix multiplication, a cornerstone operation in deep learning systems."*
...
- *"We present V-ABFT, a variance-based adaptive threshold algorithm that achieves tighter error bounds by directly modeling the verification difference."*
...
从上述表述可见，作者关注的核心矛盾是在压缩数值精度的同时保持模型能力，并以 GPT-2、LLaMA-7B、ViT 等模型为主要研究对象。
...
- *"Our method requires only $O(n)$ complexity using max/min/mean statistics, compared to A-ABFT's $O(pn)$ for finding $p$ largest values."*
