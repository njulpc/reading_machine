# 深度技术分析：QVLA: Not All Channels Are Equal in Vision-Language-Action Model's Quantization

> **论文信息**
> - **arXiv ID**: 2602.03782
> - **标题**: QVLA: Not All Channels Are Equal in Vision-Language-Action Model's Quantization
> - **作者**: Yuhao Xu, Yantai Yang, Zhenyang Fan, Yufan Liu, Yuming Li, Bing Li 等
> - **提交日期**: 2026-02-03
> - **分类**: cs.CV, cs.RO
...
本文属于**量化（Quantization）、剪枝（Pruning）、硬件加速/软硬件协同**方向的研究，提出了名为 **QVLA** 的方法。

> 论文摘要首句：*"The advent of Vision-Language-Action (VLA) models represents a significant leap for embodied intelligence, yet their immense computational demands critically hinder deployment on resource-constrained robotic platforms."*

### 1.2 一句话总结

本文提出 QVLA：However, we find that a systematic analysis of VLA model's quantization is fundamentally lacking.（摘要原文）
...
- *"We argue that naively applying uniform-bit quantization from Large Language Models (LLMs) to robotics is flawed, as these methods prioritize passive data fidelity while ignoring how minor action deviations compound into catastrophic task failures."*
...
1. 提出了可命名的新方法/框架 **QVLA**，属于量化（Quantization）、剪枝（Pruning）、硬件加速/软硬件协同方向的新方案；
...
- *"This process yields a precise, per-channel importance metric that guides a global optimization, which elegantly unifies quantization and pruning (0-bit) into a single, cohesive framework."*
- *"In the LIBERO, the quantization version of OpenVLA-OFT with our method requires only 29.2% of the original model's VRAM while maintaining 98.9% of its original performance and achieving a 1.49x speedup."*
- *"This translates to a 22.6% performance improvement over the LLM-derived method SmoothQuant."*

**摘要中出现的关键数值**（去重后）：0, 1.49x, 22.6%, 29.2%, 98.9%
...
4. 本文（QVLA）表明即通过降低权重/激活的数值精度来压缩模型体积、降低显存占用并加速推理，是大模型部署的核心技术之一——其具体设计（见第 3 节）可作为后续工作的直接参考。
